...
import { Plus, ArrowDownToLine, Pencil, Trash2, FileText, Eye } from 'lucide-react';
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      {isLoading ? <SkeletonList /> : filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((e: any) => (
            <Card key={e.id} className="shadow-sm border-none">
              <CardContent className="p-3 flex items-center gap-3">
                ...
                <span className="text-sm font-bold text-success mr-1">+{Number(e.quantidade)}</span>
                <div className="flex gap-1">
                  {e.nota_fiscal_url && (
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setViewNota(e.nota_fiscal_url)}>
      ...
      <Dialog open={!!viewNota} onOpenChange={(open) => !open && setViewNota(null)}>
        <DialogContent className="max-w-lg max-h-[80vh]">
          ...
          {viewNota && (
            viewNota.endsWith('.pdf') ? (
              <iframe src={viewNota} className="w-full h-[60vh] rounded-lg" />
            ) : (
              <img src={viewNota} alt="Nota Fiscal" className="w-full rounded-lg" />
            )
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          ...
          <form onSubmit={e => { e.preventDefault(); save.mutate(); }} className="space-y-3">
            ...
            <Button type="submit" className="w-full h-12" disabled={save.isPending || !form.produto_id}>
              {save.isPending ? 'Registrando...' : editingId ? 'Atualizar' : 'Registrar Entrada'}
            </Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <ConfirmDialog
        ...
        description="Tem certeza? A quantidade será subtraída do estoque automaticamente."
